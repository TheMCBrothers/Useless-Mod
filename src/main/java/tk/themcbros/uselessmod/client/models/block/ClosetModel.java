...
public class ClosetModel implements IBakedModel {
	...
	public List<BakedQuad> getQuads(BlockState state, Direction side, Random rand) {
		return this.getCustomModel(ClosetRegistry.CASINGS.getKeys().get(0), ClosetRegistry.BEDDINGS.getKeys().get(0), Direction.NORTH, Boolean.FALSE).getQuads(state, side, rand);
	}
	...
	public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @Nonnull Random rand,
		...
		Boolean open = data.getData(ClosetTileEntity.OPEN);
	...
	public TextureAtlasSprite getParticleTexture(@Nonnull IModelData data) {
		...
		Direction facing = data.getData(ClosetTileEntity.FACING);
		Boolean open = data.getData(ClosetTileEntity.OPEN);
		return this.getCustomModel(casing, bedding, facing, open).getParticleTexture();
	}
