...
public class UselessBigTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {

    private static final BlockState OAK_LOG = ModBlocks.USELESS_LOG.getDefaultState();
    private static final BlockState OAK_LEAVES = ModBlocks.USELESS_LEAVES.getDefaultState();

    public UselessBigTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn, boolean doBlockNotifyIn) {
    ...
    private void makeFoliage(IWorldGenerationReader worldIn, int p_208525_2_, BlockPos pos, List<UselessBigTreeFeature.FoliageCoordinates> p_208525_4_, MutableBoundingBox p_208525_5_, Set<BlockPos> changedBlocks) {
        for(UselessBigTreeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208525_4_) {
    ...
    private void makeBranches(Set<BlockPos> p_208524_1_, IWorldGenerationReader p_208524_2_, int p_208524_3_, BlockPos p_208524_4_, List<UselessBigTreeFeature.FoliageCoordinates> p_208524_5_, MutableBoundingBox p_208524_6_) {
        for(UselessBigTreeFeature.FoliageCoordinates bigtreefeature$foliagecoordinates : p_208524_5_) {
    ...
    public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox boundsIn) {
        ...
        if (i == -1) {
            ...
            int i1 = i - 5;
            List<UselessBigTreeFeature.FoliageCoordinates> list = Lists.newArrayList();
            list.add(new UselessBigTreeFeature.FoliageCoordinates(position.up(i1), l));
                    ...
                    for(int j1 = 0; j1 < k; ++j1) {
                        ...
                        if (this.makeLimb(changedBlocks, worldIn, blockpos, blockpos1, false, boundsIn) == -1) {
                            ...
                            if (this.makeLimb(changedBlocks, worldIn, blockpos2, blockpos, false, boundsIn) == -1) {
                                list.add(new UselessBigTreeFeature.FoliageCoordinates(blockpos, blockpos2.getY()));
                            }
                        }
                    }
                }
