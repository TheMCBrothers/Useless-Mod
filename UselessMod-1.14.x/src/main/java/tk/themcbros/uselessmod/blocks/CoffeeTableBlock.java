...
public class CoffeeTableBlock extends Block implements IWaterLoggable {
	...
	public BlockState getStateForPlacement(BlockItemUseContext context) {
		...
		if (blockstate.getBlock() == this) {
			return blockstate.with(TALL, Boolean.valueOf(true)).with(WATERLOGGED, Boolean.valueOf(false));
		} else {
			IFluidState ifluidstate = context.getWorld().getFluidState(blockpos);
			BlockState blockstate1 = this.getDefaultState().with(TALL, Boolean.valueOf(false)).with(WATERLOGGED,
